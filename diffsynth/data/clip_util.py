...
def load_mesh(path: str) -> trimesh.Trimesh:
    """
    Robustly loads a 3D mesh model from a specified path.

    Works for both:
    - GLB models that load as trimesh.Scene
    - PLY/GLB models that load as trimesh.Trimesh
    """
    ...
    # If it's already a Trimesh (single mesh), keep it
    if isinstance(obj, trimesh.Trimesh):
        mesh = obj

    # If it's a Scene, merge to a single mesh
    else:
        mesh = obj.dump(concatenate=True)

    # Subdivide overly large triangles for performance
    mesh = subdivide_mesh(mesh)
